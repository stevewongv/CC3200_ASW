...
#define DELETE_REQUEST_URI 	"/"


#define PUT_REQUEST_URI 	"/"
...
void parseJson(char * pMsg)
{
    ...
    pSub = cJSON_GetObjectItem(pJson, "USER_NAME");
    if(NULL == pSub){}
    strcpy(response_json[USER_NAME],pSub->valuestring);
    UART_PRINT("obj_USER_NAME : %s\n", response_json[USER_NAME]);
    // get bool from json
    pSub = cJSON_GetObjectItem(pJson, "Content");
    if(NULL == pSub){}
    strcpy(response_json[Content],pSub->valuestring);
    UART_PRINT("obj_Content : %s\n", response_json[Content]);
    cJSON_Delete(pJson);
}


/*
 * 处理接收的数据：先验证用户名（不符break）->但用户名正确时验证ID是否改变（未改变break）->实行指令
 * 用strcmp来对比信息
 */
void process_token(char response_json[][30],int ID)
{
	if(strcmp(response_json[USER_NAME],user_name)==0){
		if(id != ID){
			id = ID;
			switch (*response_json[Content]){
			case'1':
...
static int readResponse(HTTPCli_Handle httpClient)
{
	...
	lRetVal = 0;
	strcpy(content,(const char *)g_buff);
//	UART_PRINT("\n%s",g_buff);
...
static int HTTPGetMethod(HTTPCli_Handle httpClient)
{
    ...
    */
    ...
    lRetVal = readResponse(httpClient);
    parseJson(content);
    process_token(response_json,ID);
    memset(response_json,0,sizeof(char)*2*30);
    return lRetVal;
}
...
restart_ap:     lRetVal = ConnectToAP();
    if(lRetVal < 0)
    {
        LOOP_FOREVER();
    }

restart:lRetVal = ConnectToHTTPServer(&httpClient);
    if(lRetVal < 0)
    {
        goto restart_ap;
    }
